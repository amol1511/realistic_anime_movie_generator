...
- Generating images may cost inference credits on Hugging Face depending on the model and usage.""")
    ...
    HF_API_URL = st.secrets.get('HF_SDXL_URL', 'https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0')
    ...
    with st.form('movie_form'):
        ...
        image_style = st.selectbox('Style hint (affects prompts)', ['vibrant realistic anime', 'cinematic anime', 'soft watercolor anime', 'dark cyberpunk anime'])
    ...
    if submitted:
        if not HF_TOKEN:
            ...
            st.info('Splitting story into scenes and generating assets... This can take some time.')
            import re
            ...
            if not failed and images and audio_files:
                st.info('Rendering final video (this might take a bit)...')
                try:
